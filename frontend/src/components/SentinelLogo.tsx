export function SentinelLogo({ compact = false }: { compact?: boolean }) {
  return <span className="inline-flex items-center gap-3">
    ...
    {!compact && <span className="grid leading-none"><strong className="text-[17px] tracking-[-.03em] text-navy-950">Sentinel</strong><small className="mt-1 text-[9px] font-semibold uppercase tracking-[.13em] text-slate-400">Protected payment paths</small></span>}
  </span>;
}
