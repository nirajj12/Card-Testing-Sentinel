...
export function SystemStatus() {
  ...
  return <div className="system-status">
    <button className={`health-pill ${healthy ? "ready" : "error"}`} type="button" onClick={() => setOpen(!open)} aria-expanded={open}><i />{status ? (healthy ? "System healthy" : "System unavailable") : "Connecting"}</button>
    {open && <div className="system-popover">
      ...
      <StatusRow icon={BrainCircuit} label="Risk model" value={status?.model_status === "ready" ? "Ready" : "Rules-only / unavailable"}/>
