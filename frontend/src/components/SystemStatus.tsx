...
export function SystemStatus() {
  ...
  const featureCount = status?.feature_count || ACTIVE_FEATURE_COUNT;
  return <div className="system-status">
    <button className={`health-pill ${healthy ? "ready" : "error"}`} type="button" onClick={() => setOpen(!open)} aria-expanded={open} title={`Running with ${featureCount} merchant-visible behavioral signals`}><i />{status ? (healthy ? "Demo system online" : "Demo unavailable") : "Connecting"}</button>
    {open && <div className="system-popover">
      ...
      <p className="status-summary">Running with {featureCount} merchant-visible behavioral signals.</p>
