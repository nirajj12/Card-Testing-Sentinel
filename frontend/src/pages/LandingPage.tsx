  ...
  return <main>
    <section className="home-hero page-width refined-hero">
      ...
      <div className={`hero-decision-stage ${mode}`} aria-label={`${suspicious ? "Suspicious burst" : "Normal purchase"} example decision flow`}>
        ...
        <div className={`razorpay-gate ${decision === "allow" ? "open" : "closed"}`}><span>Razorpay order path</span><strong>{decision === "allow" ? "Available after ALLOW" : "Closed before order creation"}</strong><small className="hero-sequence-caption">Illustrative sequence — Replay decisions are runtime-scored and may rise or fall.</small></div>
      </div>
    </section>
