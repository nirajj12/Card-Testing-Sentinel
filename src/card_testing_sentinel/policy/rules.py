...
RULES = (
    Rule(
        ...
        2,
        "Four or more request-known attempts within 60 seconds.",
    ),
    Rule(
        "PROCESSED_VELOCITY", 1, "Five or more processed attempts within five minutes."
    ),
    ...
    Rule(
        "CARD_SWITCH_AFTER_DECLINE", 1, "A prior decline is followed by card switching."
    ),
    Rule(
        "MULTI_SESSION_PERSISTENCE",
        2,
        "At least three sessions and four prior attempts over seven days.",
    ),
    Rule(
        "SHARED_IP_INTENSITY",
        1,
        "Eight or more request-known attempts on the IP within five minutes.",
    ),
    Rule(
        "LOW_AMOUNT_WITH_DIVERSITY",
        1,
        "Low-value behavior is supporting evidence only when cards vary.",
    ),
)
...
def evaluate_rules(features: dict) -> tuple[int, list[str]]:
    conditions = (
        ...
        features["near_minimum_ratio_24h"] >= 0.5
        and features["distinct_cards_24h"] >= 2,
    )
    reasons = [
        rule.reason_code
        for rule, matched in zip(RULES, conditions, strict=True)
        if matched
    ]
    score = sum(
        rule.contribution
