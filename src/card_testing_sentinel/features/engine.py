"""Online-compatible extended feature engine."""

from collections import defaultdict
from datetime import timedelta
from statistics import mean, pstdev

from card_testing_sentinel.domain.events import EventContractError, LifecycleEvent
from card_testing_sentinel.features.base import CausalFeatureEngine as BaseFeatureEngine
from card_testing_sentinel.features.specification import EXTENDED_FEATURES
...
class CausalFeatureEngine(BaseFeatureEngine):
    """Extend frozen base state without modifying its protected source."""
    ...
    def _extended_snapshot(self, event: LifecycleEvent) -> dict[str, float]:
        ...
        return {name: float(values[name]) for name in EXTENDED_FEATURES}
    ...
    def precheck(self, event: LifecycleEvent, *, blocked: bool = False) -> dict:
        ...
        extended = self._extended_snapshot(event) if not duplicate else None
        result = super().precheck(event, blocked=blocked)
        if duplicate:
            return result
        combined = {**result, **extended}
