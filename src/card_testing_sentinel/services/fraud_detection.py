...
LOGGER = logging.getLogger("card_testing_sentinel.application")
...
class FraudDetectionService:
    ...
    def __init__(
        ...
        self.engine = CausalFeatureEngine()
        self.policy = OperationalPolicy(registry.policy)
    ...
    async def precheck_with_evidence(
        ...
        """Same real precheck path as :meth:`precheck`, additionally
        returning the allowlisted safe-evidence subset of the exact
        snapshot used for this decision. Never duplicates feature
        computation or scoring -- it only captures a read-only view of the
        snapshot :meth:`_precheck` already builds. On an idempotent replay
        no fresh snapshot exists, so the evidence dict is empty rather than
        recomputed."""
        async with self.lock:
            evidence_sink: dict = {}
            response = self._precheck(request, evidence_sink=evidence_sink)
            return response, evidence_sink
    ...
    def _precheck(
        ...
        if existing or event_existing:
            ...
            if evidence_sink is not None:
                # Return the *original* authoritative evidence captured at
                # decision time -- never recomputed, never fabricated as
                # empty just because this is a replay.
        ...
        snapshot = {
            ...
            **self.engine._extended_snapshot(event),
        }
        if evidence_sink is not None:
    ...
    def rebuild_from_persistence(self) -> None:
        self.engine = CausalFeatureEngine()
        self.policy = OperationalPolicy(self.registry.policy)
        records = [
