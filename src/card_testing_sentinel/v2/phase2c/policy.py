...
class PolicyDecision:
    """A live action and non-sensitive, stable explanation contract."""
...
@dataclass
class PolicyState:
    """Policy-only state. No labels, outcomes, or scenario metadata are stored."""

    schema_version: str = "v2c-policy-state-1"
    ...
    def from_dict(cls, payload: dict) -> PolicyState:
        if payload.get("schema_version") != "v2c-policy-state-1":
            raise ValueError("unsupported Phase 2C policy-state schema")
        restored = dict(payload)
        restored["recent_scores"] = [tuple(row) for row in payload["recent_scores"]]
        return cls(**restored)


def enumerate_candidates(config: dict) -> list[dict]:
    """Expand the declared grid in YAML order and assign stable IDs."""
    defaults = dict(config["candidate_defaults"])
    candidates: list[dict] = []
    for family, rows in config["candidate_grid"].items():
    ...
    if len(candidates) > 120:
        raise ValueError("Phase 2C candidate limit is 120")
    width = max(3, len(str(len(candidates))))
    for index, candidate in enumerate(candidates, start=1):
        candidate["candidate_id"] = f"phase2c_{index:0{width}d}"
...
def candidate_enumeration_sha256(candidates: list[dict]) -> str:
    encoded = json.dumps(candidates, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(encoded.encode()).hexdigest()


def load_candidates(config_path) -> tuple[dict, list[dict]]:
...
class StatefulPolicy:
    ...
    Risk is a decayed sum of calibrated probabilities. Scores are retained only
    ...
    def decide(
        ...
        *,
        ...
        digest = self._event_digest(
            event_id, timestamp, session_id, probability, snapshot
        )
        ...
        effective_probability = float(probability)
        if (
            self.candidate["family"] == "checkout_protected"
            and state.checkout_protection_remaining > 0
        ):
            effective_probability *= float(self.candidate["checkout_risk_multiplier"])
            state.checkout_protection_remaining -= 1
        state.accumulated_risk += effective_probability
        ...
        state.consecutive_review = (
            ...
            if effective_probability >= review_threshold
            else 0
        )
        state.consecutive_strong = (
            state.consecutive_strong + 1
            if effective_probability >= block_threshold
            else 0
        )
        state.recent_scores.append((timestamp.isoformat(), effective_probability))
        boundary = (
    ...
    @classmethod
    def deserialize(cls, encoded: str) -> StatefulPolicy:
