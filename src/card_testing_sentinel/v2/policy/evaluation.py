...
def _allow_all_parity(raw: pd.DataFrame, frozen: pd.DataFrame) -> dict:
    ...
    return {"precheck_rows": len(expected), "features": len(MODEL_FEATURES), "maximum_absolute_difference": maximum, "passed": True}


def _detailed_sequential_metrics(devices: pd.DataFrame) -> dict:
    result = {"all_devices": int(len(devices)), "all_attackers": int(devices.label.sum())}
    ...
    for limit in (1, 3, 5, 10):
        result["within_request"][str(limit)] = {
            "review_or_higher": {
                "numerator": int(attackers.first_review_or_higher_request.le(limit).sum()),
    ...
    for subtype, group in attackers.groupby("attack_subtype"):
        result["by_subtype"][str(subtype)] = {
            ...
            "within_request": {
                str(limit): {
                    "review_or_higher": int(group.first_review_or_higher_request.le(limit).sum()),
    ...
    result["legitimate"] = {"overall": {"review_or_higher": proportion(legitimate, "review_or_higher"), "block": proportion(legitimate, "blocked")}}
    for scenario, group in legitimate.groupby("scenario_tag"):
        result["legitimate"][str(scenario)] = {"review_or_higher": proportion(group, "review_or_higher"), "block": proportion(group, "blocked")}
    ...
    result["potentially_preventable_later_requests_upper_bound"] = int(devices.potentially_preventable_later_requests_upper_bound.sum())
    return result
...
def _benchmark(artifact, row: pd.DataFrame, candidate: dict, rule_score: int) -> dict:
    ...
    def values(samples):
        return {"p50_ms": float(np.percentile(samples, 50)), "p95_ms": float(np.percentile(samples, 95)), "p99_ms": float(np.percentile(samples, 99)), "sample_count": len(samples)}
    return {"model_precheck": values(model_times), "policy_decision": values(policy_times), "scope": "warm local Python call; excludes future HTTP, network and durable state", "runtime": platform.platform()}


def run_validation_policy_phase(root: Path = ROOT) -> dict:
    training_freeze = verify_training_freeze()
    features, raw, access = open_validation()
    parity = _allow_all_parity(raw, features)
    serialized_artifact = joblib.load(root / "artifacts/v2/models/calibrated_model.joblib")
    ...
    contract = splits.loc[splits.split.eq("validation"), ["device_id", "population", "attack_subtype", "scenario_tag", "label"]].copy()
    observed_denominators = {"overall_legitimate": int(contract.label.eq(0).sum())}
    observed_denominators.update({str(key): int(value) for key, value in contract.loc[contract.label.eq(0)].groupby("scenario_tag").size().items()})
    for name, budget in policy_config["budgets"].items():
        ...
        if budget["review_or_higher_allowance"] != int(np.floor(budget["denominator"] * budget["review_or_higher_rate"])):
            raise RuntimeError(f"review allowance rounding mismatch for {name}")
        if budget["block_allowance"] != int(np.floor(budget["denominator"] * budget["block_rate"])):
    ...
    for candidate in enumerate_policy_grid(policy_config):
        ...
        candidate_rows.append(
            {
                **candidate,
                ...
                "macro_subtype_review_coverage": metrics["macro_subtype_review_coverage"],
                ...
                "median_processed_authorizations_before_first_action": metrics["median_processed_authorizations_before_first_action"],
        ...
        cached[candidate["candidate_id"]] = (candidate, decisions, devices, metrics, objective)
    ...
    if not feasible:
        raise RuntimeError("no frozen validation policy candidate satisfies every budget")
    ...
    if list(decisions.action) != list(repeat_decisions.action) or not np.allclose(decisions.calibrated_probability, repeat_decisions.calibrated_probability, equal_nan=True, rtol=0, atol=1e-12):
    ...
    static = {
        ...
        "calibrated": probability_metrics(features.label, calibrated_probability, weights),
        "unit": "precheck rows with device evaluation weights",
    }
    reliability_table(features.label.to_numpy(), raw_probability, weights).to_csv(root / "artifacts/v2/metrics/validation_reliability_raw.csv", index=False)
    reliability_table(features.label.to_numpy(), calibrated_probability, weights).to_csv(root / "artifacts/v2/metrics/validation_reliability_calibrated.csv", index=False)
    family_comparison = {}
    for family in ("rules_only", "ml_only", "combined"):
        ...
        if available:
            ...
            family_comparison[family] = {"candidate": representative[0], "metrics": representative[3], "objective_tuple": list(representative[4])}
        else:
    ...
    metrics_payload = {
        ...
        "family_feasible_counts": {str(key): int(value) for key, value in table.loc[table.feasible].groupby("family").size().items()},
        ...
        "latency": latency,
        "intervention_semantics": "block_current_attempt suppresses its outcome and all later device state; review processes the recorded outcome; later requests after block are counterfactual upper-bound evidence only",
    }
    ...
    policy_payload = {
        ...
        "training_freeze_sha256": sha256_file(root / "artifacts/v2/training/training_freeze.json"),
        "policy": selected,
        ...
        "model_artifact_sha256": sha256_file(root / "artifacts/v2/models/calibrated_model.joblib"),
    }
    ...
    report = [
        ...
        "",
        f"Training freeze was created at `{access['training_freeze_created_utc']}`; first validation access was `{access['first_validation_access_utc']}`.",
        "",
        f"Allow-all parity passed for {parity['precheck_rows']:,} precheck rows and {parity['features']} causal features (maximum absolute difference {parity['maximum_absolute_difference']:.3g}).",
        "",
        f"The complete frozen grid contained {len(table)} candidates; {int(table.feasible.sum())} satisfied every overall and subgroup allowance.",
        f"Selected `{selected['candidate_id']}` (`{selected['family']}`) with objective tuple `{list(objective)}`.",
        "",
        f"Attacker review-or-higher: {sequential['attacker_review_or_higher']['numerator']}/{sequential['attacker_review_or_higher']['denominator']}; blocked: {sequential['attacker_block']['numerator']}/{sequential['attacker_block']['denominator']}.",
        f"Legitimate review-or-higher: {selected_metrics['legitimate_review_or_higher']}/1700; blocked: {selected_metrics['legitimate_blocks']}/1700.",
        f"Potentially preventable later requests (offline upper bound, not observed prevented fraud): {sequential['potentially_preventable_later_requests_upper_bound']}.",
    ]
    ...
    with mlflow.start_run(run_name="validation-policy-freeze"):
        mlflow.log_params({"candidate_id": selected["candidate_id"], "family": selected["family"], "training_freeze_sha256": policy_payload["training_freeze_sha256"]})
        mlflow.log_metrics({"feasible_candidates": int(table.feasible.sum()), "worst_subtype_review_coverage": selected_metrics["worst_subtype_review_coverage"], "legitimate_blocks": selected_metrics["legitimate_blocks"]})
        mlflow.log_artifact(str(policy_path), artifact_path="policy")
    return {"selected_policy": selected, "objective_tuple": list(objective), "candidate_count": int(len(table)), "feasible_candidate_count": int(table.feasible.sum()), "metrics_path": str(metrics_path), "policy_path": str(policy_path)}
