...
def _budget_excess(budgets: dict) -> tuple[int, list[dict]]:
    ...
    for subgroup, values in budgets.items():
        review_excess = max(
            0, values["review_or_higher"] - values["review_allowance"]
        )
...
def write_blocked_report(root: Path = ROOT) -> dict:
    ...
    if table.empty or bool(table.feasible.any()):
        raise RuntimeError("blocked report requires a complete grid with no feasible policy")
    ...
    payload = {
        ...
        "family_candidate_counts": {
            str(key): int(value) for key, value in table.groupby("family").size().items()
        },
    ...
    report = [
        ...
        "",
        f"The training freeze was created at `{freeze['created_utc']}` and validation was first opened at `{access['first_validation_access_utc']}`.",
        "",
        f"All {len(table)} candidates in the frozen grid were evaluated with intervention-aware raw lifecycle replay. Zero candidates met every overall and subgroup allowance, so no V2 policy was selected or frozen.",
        "",
        "## Exact closest boundaries",
        "",
        f"- Rules-only `{rules['candidate_id']}` exceeded only normal_bad_luck review-or-higher: 6/100 against an allowance of 5/100. Its worst-subtype attacker review coverage was {rules['worst_subtype_review_coverage']:.4f}.",
        f"- ML-only `{ml['candidate_id']}` was {ml['total_devices_over_allowances']} device-count units over frozen subgroup allowances; see the machine-readable failures for exact review/block counts.",
        f"- Combined `{combined['candidate_id']}` was {combined['total_devices_over_allowances']} device-count units over frozen subgroup allowances.",
        "",
    ...
    hashes = {
        "artifacts/v2/metrics/validation_policy_candidates.csv": sha256_file(table_path),
        "artifacts/v2/metrics/validation_blocked_metrics.json": sha256_file(metrics_path),
