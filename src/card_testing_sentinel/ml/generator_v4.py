...
from card_testing_sentinel.ml.scenarios_v4 import (
    CRITICAL_SCENARIOS,
    SCENARIOS_V4,
    ScenarioV4,
)
...
def build_merchants_v4(config: dict) -> list[MerchantProfile]:
    ...
    extras = list(
        rng.choice(names, size=max(0, count - len(names)), p=weights / weights.sum())
    )
...
class GeneratorV4:
    ...
    def _generate_actor(
        ...
        device_count = int(
            surface.get(
                "devices",
                self.rng.integers(scenario.devices[0], scenario.devices[1] + 1),
            )
        )
        devices = [
        ...
        base_affinity = float(
            self.config["merchants"]["kinds"][merchant.kind].get("login_affinity", 0.75)
        )
        mult = float(
            self.rng.uniform(scenario.login_multiplier[0], scenario.login_multiplier[1])
        )
        ...
        episode_count = int(
            surface.get(
                "episodes",
                self.rng.integers(scenario.episodes[0], scenario.episodes[1] + 1),
            )
        )
        ...
        start_offset_days = float(
            self.rng.uniform(0.5, max(1.0, self.window_days - 10.0))
        )
        ...
        history_customers = (
            ...
            if scenario.name == "shared_household_device"
                ...
                [primary_customer]
                if scenario.age_pairing == "established_burst"
                else []
            )
        )
        for history_customer in history_customers:
            ...
            hist_card = (
                instruments[0]
                if instruments
                else new_instrument(self.rng, self.instrument_config, 1.0)
            )
            ...
            for hist_index in range(history_count, 0, -1):
                ...
                hist_session = (
                    f"{self.prefix}ses_hist_{hist_dev}_"
                    f"{history_customer.customer_id}_{hist_index:02d}"
                )
                common = dict(
                    request_id=hist_req_id, device_id=hist_dev, session_id=hist_session
                )
                events.append(
                    blank_event(
                        ...
                        amount=merchant.draw_amount(self.rng),
                        currency="INR",
                ...
                events.append(
                    blank_event(
                        ...
                        hist_time + timedelta(seconds=2),
                        f"{self.prefix}evt_{self._next('event'):08d}",
                        authorization_result="approved",
                        payment_method="card",
                        card_last4=hist_card.last4,
                        card_network=hist_card.network,
                        card_type=hist_card.card_type,
                        card_issuer=hist_card.issuer,
                        international=hist_card.international,
                        **common,
                    )
                )
                events.append(
                    blank_event(
                        "checkout_completion",
                        hist_time + timedelta(seconds=15),
                        f"{self.prefix}evt_{self._next('event'):08d}",
                        **common,
                    )
                )
        ...
        repeated_amount = float(
            surface.get(
                "amount",
                499.0
                if scenario.name == "subscription_dunning_hard"
        ...
        for ep_idx in range(episode_count):
            ...
            session_id = (
                f"{self.prefix}ses_{device_id}_{ep_idx:02d}_"
                f"{int(self.rng.integers(100, 999))}"
            )
            ...
            attempt_count = int(
                surface.get(
                    "attempts",
                    self.rng.integers(scenario.attempts[0], scenario.attempts[1] + 1),
                )
            )
            _last_card = instruments[0] if instruments else None
            ...
            for att_idx in range(attempt_count):
                ...
                if int(surface.get("devices", 1)) > 1:
                    ...
                    used_devices.add(device_id)
                    session_id = (
                        f"{self.prefix}ses_{device_id}_{ep_idx:02d}_{att_idx:02d}"
                    )

                # Session rotation within episode
                if self.rng.random() < float(
                    self.rng.uniform(
                        scenario.session_rotation[0], scenario.session_rotation[1]
                    )
                ):
                    session_id = (
                        f"{self.prefix}ses_{device_id}_{ep_idx:02d}_{att_idx:02d}_"
                        f"{int(self.rng.integers(1000, 9999))}"
                    )

                # IP rotation
                if (
                    scenario.name != "cgnat_mobile_ip_storm"
                    and self.rng.random()
                    < float(
                        self.rng.uniform(
                            scenario.ip_rotation[0], scenario.ip_rotation[1]
                        )
                    )
                ):
                    ip_addr = self._draw_ip(episode_customer, scenario.ip_pool)
                ...
                if scenario.card_diversity_mode == "high_rotation":
                    card = new_instrument(
                        ...
                        float(
                            self.rng.uniform(
                                scenario.method_validity[0], scenario.method_validity[1]
                            )
                        ),
                    )
                    ...
                    if not instruments or self.rng.random() > float(
                        self.rng.uniform(
                            scenario.instrument_reuse[0], scenario.instrument_reuse[1]
                        )
                    ):
                        card = new_instrument(
                            self.rng,
                ...
                if amount_style == "low":
                    amount = float(
                        surface.get(
                            "amount", round(float(self.rng.uniform(1.0, 49.0)), 2)
                        )
                    )
                    ...
                    amount = float(
                        surface.get("amount", merchant.draw_amount(self.rng))
                    )

                req_id = f"{self.prefix}req_{self._next('request'):07d}"
                is_campaign = (
                    merchant.in_campaign(current_time) or scenario.prefers_campaign
                )
                ...
                outcome_time = current_time + timedelta(
                    seconds=int(self.rng.integers(1, 4))
                )
                events.append(
                    blank_event(
                        "authorization_outcome",
                ...
                if result == "approved" and self.rng.random() < float(
                    self.rng.uniform(
                        scenario.checkout_completion[0], scenario.checkout_completion[1]
                    )
                ):
                    checkout_time = outcome_time + timedelta(
                        seconds=int(self.rng.integers(5, 20))
                    )
                    events.append(
                        blank_event(
                            "checkout_completion",
                    ...
                    if not surface and self.rng.random() > float(
                        self.rng.uniform(
                            scenario.continue_after_success[0],
                            scenario.continue_after_success[1],
                        )
                    ):
                        break
                ...
                gap_sec = float(
                    surface.get(
                        ...
                        lognormal_gap(
                            ...
                            float(
                                self.rng.uniform(
                                    scenario.gap_seconds[0], scenario.gap_seconds[1]
                                )
                            ),
                            float(
                                self.rng.uniform(
                                    scenario.gap_spread[0], scenario.gap_spread[1]
                                )
                            ),
                        ),
                    )
            ...
            ep_gap_days = float(
                self.rng.uniform(
                    scenario.episode_gap_days[0], scenario.episode_gap_days[1]
                )
            )
    ...
    def generate(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for sc_name in CRITICAL_SCENARIOS:
            ...
            while scenario_counts.get(sc_name, 0) < sc_target:
                ...
                scenario_counts[sc_name] = scenario_counts.get(sc_name, 0) + len(
                    actor_labels
                )
        ...
        if self.split_name == "validation":
            cf_designs = [
                ...
                (
                    "success_camouflage_v4",
                    "normal_returning_customer",
                    5,
                    1,
                    180.0,
                    1499.0,
                ...
                (
                    "merchant_normal_amount_attack_v4",
                    "genuine_wallet_cycling",
                    3,
                    1,
                    200.0,
                    45000.0,
            ...
            for pair_idx, design in enumerate(cf_designs, start=1):
                ...
                att_sc_name, leg_sc_name, attempts, devices, gap_seconds, amount = (
                    design
                )
                ...
                att_evs, att_labs = self._generate_actor(
                    att_sc,
                    force_merchant=matched_merchant,
                    cf_pair_id=pair_id,
                    cf_role="attack",
                    shared_surface=shared_surface,
                )
                counts["attack"] += len(att_labs)
                scenario_counts[att_sc_name] = scenario_counts.get(
                    att_sc_name, 0
                ) + len(att_labs)
                ...
                leg_evs, leg_labs = self._generate_actor(
                    ...
                    force_merchant=matched_merchant,
                    cf_pair_id=pair_id,
                    cf_role="legitimate_twin",
                    shared_surface=shared_surface,
                )
                counts["legitimate"] += len(leg_labs)
                scenario_counts[leg_sc_name] = scenario_counts.get(
                    leg_sc_name, 0
                ) + len(leg_labs)
        ...
        legit_pool = [
            s for s in self.scenarios.values() if s.population == "legitimate"
        ]
        ...
        while (
            counts["attack"] < attack_device_target
            or counts["legitimate"] < legit_device_target
        ):
            if counts["attack"] >= attack_device_target:
                sc = legit_pool[
                    int(
                        self.rng.choice(
                            len(legit_pool), p=leg_weights / leg_weights.sum()
                        )
                    )
                ]
                ...
                sc = attack_pool[
                    int(
                        self.rng.choice(
                            len(attack_pool), p=att_weights / att_weights.sum()
                        )
                    )
                ]
            else:
            ...
            target_for_population = (
                attack_device_target
                if sc.population == "attack"
                else legit_device_target
            )
            ...
            if len(actor_labels) > remaining:
                ...
                continue
            counts[sc.population] += len(actor_labels)
            scenario_counts[sc.name] = scenario_counts.get(sc.name, 0) + len(
                actor_labels
            )
            events.extend(actor_events)
            labels.extend(actor_labels)
...
def generate_dataset_v4(config: dict) -> dict:
    ...
    val_gen = GeneratorV4(
        config, "validation", splits["validation"], merchants, val_start
    )
    ...
    raw_events = raw_events.sort_values("timestamp", kind="mergesort").reset_index(
        drop=True
    )
...
def build_manifest_v4(config: dict, bundle: dict) -> dict:
    ...
    cf_pairs = labels.dropna(subset=["counterfactual_pair_id"])[
        "counterfactual_pair_id"
    ].nunique()
    ...
    return {
        ...
        "splits": {
            split: {
                "devices": int(
                    labels.loc[labels.split.eq(split), "device_id"].nunique()
                ),
                "requests": int(
                    raw.loc[
                        raw.split.eq(split)
                ...
                "attack_devices": int(
                    labels.loc[
                        labels.split.eq(split) & labels.label.eq(1), "device_id"
                    ].nunique()
                ),
                "legitimate_devices": int(
                    labels.loc[
                        labels.split.eq(split) & labels.label.eq(0), "device_id"
                    ].nunique()
                ),
            }
