...
from card_testing_sentinel.common.io import atomic_write_json, atomic_write_text
...
def write_development_bundle(config: dict, output_dir: Path) -> dict:
    raw, splits = generate_frames(config)
    split_map = splits.set_index("device_id")["split"]
    tagged = raw.assign(split=raw.device_id.map(split_map))
    feature_parts = []
    for split, events in tagged.groupby("split", sort=True):
