        ...
        rows.append(
            {
                ...
                "observed_rate": float(np.average(labels[mask], weights=weights[mask]))
                if mask.any()
                else math.nan,
            }
        )
...
def probability_metrics(labels, probabilities, weights, threshold=None) -> dict:
    ...
    if threshold is not None:
        ...
        result.update(
            ...
            precision=float(
                precision_score(
                    labels, predicted, sample_weight=weights, zero_division=0
                )
            ),
            recall=float(
                recall_score(labels, predicted, sample_weight=weights, zero_division=0)
            ),
            f1=float(
                f1_score(labels, predicted, sample_weight=weights, zero_division=0)
            ),
        )
    return result
...
def wilson_interval(
    numerator: int, denominator: int, z: float = 1.96
) -> tuple[float, float]:
    if denominator == 0:
