...
def test_multi_device_leakage_groups_never_straddle_folds() -> None:
    devices = pd.DataFrame({
        ...
        "leakage_group_id": ["actor-1", "actor-1", "actor-2", "actor-3", "actor-3", "actor-4"],
    })
...
def test_fold_assignment_is_row_order_independent() -> None:
    ...
    second = make_leakage_group_folds(
        devices.sample(frac=1.0, random_state=7), 2, 99
    ).set_index("device_id").fold
