    ...
    frame = pd.DataFrame(
        {
            ...
            "label": [0, 0, 0, 1, 1, 1],
            "scenario_tag": ["normal", "normal", "normal", "attack", "attack", "attack"],
        }
    )
...
def test_policy_grid_is_complete_deterministic_and_strict():
    config = {
        "families": {
            ...
            "ml_only": {"review_thresholds": [0.15, 0.25, 0.35, 0.45, 0.55], "block_thresholds": [0.6, 0.7, 0.8, 0.9]},
            "combined": {"review_thresholds": [0.25, 0.4], "block_thresholds": [0.7, 0.85], "review_scores": [2, 3], "block_support_scores": [2, 3]},
        }
    }
...
def _contract():
    return pd.DataFrame(
        [{"device_id": "device-1", "population": "attack", "attack_subtype": "patient", "scenario_tag": "attack_patient", "label": 1}]
    )
...
def test_current_block_scores_once_suppresses_outcome_and_later_state():
    ...
    assert list(decisions.action) == ["block_current_attempt", "counterfactual_after_block"]
...
def test_model_contract_contains_only_explicit_numeric_features():
    forbidden = {"device_id", "session_id", "label", "attack_subtype", "scenario_tag", "timestamp"}
